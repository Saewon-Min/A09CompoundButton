...
/*
각 버튼을 선택(체크) 했을때 감지하기 위해
1. CompoundButton.OnCheckedChangeListener 인터페이스를 구현한 후
2. onCheckedChanged() 메소드를 오버라이딩 한다. (onCreate()메소드 아래에 기술함)
 */
public class MainActivity extends AppCompatActivity implements
    ...
    private String[] items = {
            "소녀시대","트와이스","마마무","블랙핑크","에이핑크"
                ,"오마이걸","에스파"
            };

    // 선택한 항목에 대한 결과 출력용 변수(라디오, 스피너, 토글버튼, 스위치)
    private String radioChecked = "여성";
    private String spinnerSelected = "에스파";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 결과 출력용 텍스트뷰를 얻어와서 글자크기와 폰트를 설정함
        ...
        radioGroup.setOnCheckedChangeListener(
                new RadioGroup.OnCheckedChangeListener() {
                    /*
                    라디오 그룹을 통해 선택된 라디오 버튼의 id값이 매개변수로
                    전달된다.
                    ...
                    public void onCheckedChanged(RadioGroup radioGroup, int i) {
                        ...
                        /*
                        안드로이드에서 로그를 확인할 때는 Log.로그종류(태그명,출력값);
                        형태로 사용한다. 로그의 종류는 info, debug, error 등이 있다.
                        ...
                        // 해당 라디오 버튼의 텍스트를 얻어와서 토스트로 출력한다.
        ...
        스피너 위젯을 얻어온 후 어뎁터 객체와 연결함
            어뎁터 객체 사용 형식]
                ArrayAdapter<타입매개변수>(context, 모양, 항목);
        ...
        /*
        앱 실행시 디폴트로 선택한 항목을 지정하려면 아래와 같이 Java에서 처리한다.
